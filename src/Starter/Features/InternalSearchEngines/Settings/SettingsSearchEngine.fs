namespace Starter.Features.InternalSearchEngines.Settings
...
type SettingsSearchEngine(baseConfig, searchEngines) =
    inherit StaticSearchEngine("", "", unbox null)

    let vm = new ViewModels.SettingsWindowViewModel(baseConfig, searchEngines)
    let mutable window = Views.SettingsWindow(DataContext = vm)
    ...
    override this.Id = id
    override this.Icon = Icons.settingsIcon
    override this.LoadResults() = struct (results :> _ seq, Observable.Empty()) |> Task.singleton
        ...
        | :? SettingsSearchResult as se ->
            Dispatcher.UIThread.Post(fun () ->
                ...
                try
                    ...
                    window.Activate()
                with _ ->
                    window <- Views.SettingsWindow(DataContext = vm)
                    window.Show()
            )
        | _ -> ()
