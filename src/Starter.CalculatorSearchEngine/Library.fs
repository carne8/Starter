...
type SearchResult =
    ...
    interface ICustomSearchResult with
        member this.Id = null
        member this.Name = ""
        member this.Description = this.Desc
        member this.Keywords = Array.empty
        member this.Icon = Icon.icon
        member this.ShowIfNoActivator = true
        member this.ActivatorFilter = Array.empty
        member this.Control = this.LaTeX

type MathView() as this =
    inherit CSharpMath.Avalonia.MathView()
...
type CalculatorSearchEngine() =
    let control = MathView(FontSize = 18f)
        ...
        member this.Search(query, ct, activator) =
            voption {
                ...
                Dispatcher.UIThread.Post(fun () ->
                    control.LaTeX <- LaTeX.fromExpression expr
                )

                return { Desc = desc; LaTeX = control }
            }
            |> ValueOption.map (fun r -> struct (r :> ISearchResult |> Seq.singleton, Observable.Empty()))
